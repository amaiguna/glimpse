...
| 002 | [Finder / Grep ファジーマッチのハイライト表示](002-fuzzy-match-highlight.md) | Approved | Finder ペイン + Grep 左ペイン (include 非空時) でファジーマッチ位置を ANSI ハイライト。`FuzzyFilter` の `MatchedIndexes` を render に流す。grep preview と同じ ANSI を使うが定数は分離 (将来付け替え容易化)。 |
